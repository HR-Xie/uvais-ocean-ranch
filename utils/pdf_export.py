...
# ── step 1: markdown → plain structured text ───────────────────────────
def _md_to_plain(md):
    """Convert markdown to a list of (style, text) tuples."""
    lines = md.split("\n")
    result = []
    in_table = False
...
# ── safe emoji filter ──────────────────────────────────────────────────
def _filter_emoji(s):
    """Remove emoji & other symbols that CJK fonts lack, keep CJK+ASCII."""
    result = []
    for c in s:
        cat = unicodedata.category(c)
        cp = ord(c)
        # Drop variation selectors (U+FE00–U+FE0F, U+E0100–U+E01EF)
...
# ── step 3: CJK-aware line wrapping ────────────────────────────────────
def _cjk_width(c):
    w = unicodedata.east_asian_width(c)
    return 2 if w in ("W", "F") else 1

def _wrap(text, max_w):
    """Wrap text to max_w display units. Returns list of lines."""
    lines = []
...
def _render_table_row(pdf, cells, col_w, font, page_h, b_margin):
    """Render a single table row with text-wrapped cells. All cells stay aligned."""
    TABLE_FONT_SIZE = 9
    LINE_HT = 5      # line height
    PAD_X = 1.2       # horizontal padding per cell
    PAD_Y = 1.5       # vertical padding

    # How many display units fit in one cell? 1mm ≈ 1.35 display units at 9pt
    ...
    # Page break if row would extend past bottom margin
...
def markdown_to_pdf(md_text: str) -> bytes:
    # Strip any preamble before the first # heading (LLM may add unwanted lead-in)
    ...
    pdf = FPDF()
    # Bottom margin = 22mm so content stays clear of the footer line at -15mm
    ...
    pdf.add_page()
    page_h = pdf.h   # page height in mm
    b_margin = 22    # must match set_auto_page_break margin

    eff_w = pdf.w - pdf.l_margin - pdf.r_margin
    max_w = 90  # ~45 CJK chars at 11pt
    tbl_cols = 0
    tbl_col_w = 0
    row_buffer = []  # accumulate cells for current table row
    ...
    # Footer line at 15mm from bottom, well above content margin (22mm)
    pdf.set_y(-16)
